...
public class RegularLogic implements Logic {
	  ...
	  public  RegularLogic(Board b) {
		   ...
		   * @return ArrayList with all the slots available to place the given tag
		   */
	  public ArrayList<Slot> SlotsToPlace(CellStatus cellStatus) {
		  ...
		  ArrayList<Slot> overallSlotsToPlaceList = new ArrayList<Slot>(64);
		     ...
		     */
		  ArrayList<Slot> temp = b.GetSlotsOfPlayer(cellStatus);
	  ...
	  public void FlipSlots(int row, int col, CellStatus flipTo) {
		  ...
		  for (int i = 0; i < slotsToFlip.size(); i++) {
		      int locationToOmit = slotsToFlip.get(i).LocationInArrayList(b.GetSlotsOfPlayer(flipFrom));
		      ArrayList<Slot> newSlots = new ArrayList<Slot>(64);
		      // if locationToOmit exist
		      if (locationToOmit != -1) {
		        // copy every players slot to a new ArrayList without slot at [location to omit]
		      ...
		      // update the Board players slots to the new ArrayList
		   ...
		   * @param row
		   * @param col
		   * @return ArrayList with all the possible slots to another tag (same status) unlocked because of
		   * the given tag
		   */
		 ...
		 * @param slotsArrayList will contain list of slots.
		 ...
		 *
		 */
		 private void OneDierectionSlots(int row, int col, int nextRow, int nextCol, ArrayList<Slot> slotsArrayList,
				...
				if (this.board.LegalPlaceInBoard(row,col) &&
					  ...
					  if (check == true) {
						  slotsArrayList.add(new Slot(row, col, enemy));
						  RecursiveCheckNextCell(nextRow, nextCol, slotsArrayList,
							  enemy, player,rowOperation, colOperation,
					                              player);
					  } else {
						  RecursiveCheckNextCell(nextRow, nextCol, slotsArrayList, enemy,
		   ...
		   * @param row
		   * @param col
		   * @return ArrayList with slots we have to flip because of the placement
		   * of the given tag
		   */
		 ...
		 private ExtraCellValidateResult FlagCheckToFlipNextCell(int row, int col, CellStatus currentTag, CellStatus tagsLookingFor) {
		  ...
		  /**
		   * Checks if a given cell is good to place tag and add it to the possible slot ArrayList
		   * @param rowToCheck
		   * @param colToCheck
